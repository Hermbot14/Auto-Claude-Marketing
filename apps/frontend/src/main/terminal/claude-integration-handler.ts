 ...
 */
function pathsAreEquivalent(path1: string | undefined, path2: string | undefined): boolean {
  if (!path1 || !path2) {
    return path1 === path2;
  }
...
export function buildClaudeShellCommand(
  ...
  switch (config.method) {
    ...
    default:
      // For default method, skip PATH prefix since terminal already has augmented PATH
      // The PTY is spawned with getAugmentedEnv() which includes all necessary paths
...
export function invokeClaude(
  ...
  try {
    ...
    // Only build cd command if terminal is not already in the target directory
    // The PTY is spawned with the correct cwd, so cd is redundant if paths match
    const needsCd = !pathsAreEquivalent(cwd, terminal.cwd);
    const cwdCommand = needsCd ? buildCdCommand(cwd, terminal.shellType) : '';
